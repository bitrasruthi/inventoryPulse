...
export function getMe(config?: AxiosRequestConfig | undefined): Promise<Types.Anonymous2> {
    ...
    return getAxios().request(options_).catch((_error: any) => {
        ...
        return processGetMe(_response);
    });
}

function processGetMe(response: AxiosResponse): Promise<Types.Anonymous2> {
    const status = response.status;
    let _headers: any = {};
    if (response.headers && typeof response.headers === "object") {
        for (let k in response.headers) {
    ...
    if (status === 200) {
        ...
        return Promise.resolve<Types.Anonymous2>(result200);
...
let _requestConfigGetMe: Partial<AxiosRequestConfig> | null;
export function getGetMeRequestConfig() {
  return _requestConfigGetMe;
}
export function setGetMeRequestConfig(value: Partial<AxiosRequestConfig>) {
  _requestConfigGetMe = value;
}
export function patchGetMeRequestConfig(patch: (value: Partial<AxiosRequestConfig>) => Partial<AxiosRequestConfig>) {
  _requestConfigGetMe = patch(_requestConfigGetMe ?? {});
}