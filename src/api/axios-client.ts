...
export class AuthControllerClient {
     ...
     */
    setUserCustomerRole(cusId: string, roleId: number, cancelToken?: CancelToken | undefined): Promise<SelectCustomerRoleResponseDto> {
    ...
    protected processSetUserCustomerRole(response: AxiosResponse): Promise<SelectCustomerRoleResponseDto> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (let k in response.headers) {
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200  = _responseText;
            result200 = SelectCustomerRoleResponseDto.fromJS(resultData200);
            return Promise.resolve<SelectCustomerRoleResponseDto>(result200);

        } else if (status !== 200 && status !== 204) {
            const _responseText = response.data;
            return throwException("An unexpected server error occurred.", status, _responseText, _headers);
        }
        return Promise.resolve<SelectCustomerRoleResponseDto>(null as any);
    }
